...
// encodeFields encodes fields.
func (s *UpdateEmailOK) encodeFields(e *jx.Encoder) {
	{
		e.FieldStart("email")
		s.Email.Encode(e)
	}
...
var jsonFieldsNameOfUpdateEmailOK = [1]string{
	0: "email",
}
...
func (s *UpdateEmailOK) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode UpdateEmailOK to nil")
	}
	var requiredBitSet [1]uint8
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		return errors.Wrap(err, "decode UpdateEmailOK")
	}
	// Validate required fields.
	var failures []validate.FieldError
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfUpdateEmailOK) {
					name = jsonFieldsNameOfUpdateEmailOK[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
