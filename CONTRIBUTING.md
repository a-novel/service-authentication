...
During registration or password reset flows, the service sends emails containing verification codes. To extract these
programmatically:

```bash
# Get the latest email and extract its ID
EMAIL_ID=$(curl -s http://localhost:4014/api/v1/messages | jq -r '.messages[0].ID')

# Get the full email content
curl "http://localhost:4014/api/v1/message/$EMAIL_ID"

# Or view directly in browser
open "http://localhost:4014/view/$EMAIL_ID.html"
```
...
│  • Route definitions, CORS, auth middleware         │
└──────────────────────────┬──────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────┐
│                    Service Layer                    │
...
- Bump the version number for releases
